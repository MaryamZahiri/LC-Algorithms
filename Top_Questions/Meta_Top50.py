...
##############################################################
# 1. 1249. Minimum Remove to Make Valid Parentheses *P @2 pass
...
# Without stack - 2 pass - counter - right to left ( - left to right )
class Solution:
...
def merge_intervals_with_graph(intervals): # intervals = [[1, 3], [2, 6], [8, 10]]
    # 1) Build the graph
    graph = defaultdict(list)

    def overlaps(a, b):
        # Check if two intervals overlap
        # Two intervals [s1, e1], [s2, e2] overlap if s1 <= e2 and s2 <= e1
        return a[1] >= b[0] and b[1] >= a[0]

    # Create edges in the graph for overlapping intervals
    for i, (start1, end1) in enumerate(intervals): 
        for j, (start2, end2) in enumerate(intervals):
            if i != j and overlaps((start1, end1), (start2, end2)):
                graph[i].append(j) # {0: [1], 1: [0], 2: []}

    # 2) Find connected components (overlapping intervals)
    visited = set()
    merged = []
    ...
    for i in range(len(intervals)): # [[0, 1], [2]]
        if i not in visited:
            component = dfs(i)
            # Merge intervals in the component
            merged.append([
                min(intervals[idx][0] for idx in component),  # Minimum start
                max(intervals[idx][1] for idx in component)   # Maximum end
            ])

    # 3) Sort the merged intervals by start time, if needed
    merged.sort(key=lambda x: x[0])
    return merged # [[1, 6], [8, 10]]

# Example usage
intervals = [[1, 3], [2, 6], [8, 10], [15, 18]]
result = merge_intervals_with_graph(intervals)
...
class Solution:
    def calculate(self, s: str) -> int:
        s = s.replace(" ", "")  # remove spaces for convenience
        s += '+'  # sentinel operator to ensure the last number is processed
        
        res = 0
        last = 0
        num = 0
        op = '+'  # assume a leading '+' for the first number
        
        for char in s:
            if char.isdigit():
                num = num * 10 + int(char)
            else:
                # Apply the previous operator to 'last' and possibly 'res'
                if op == '+':
                    res += last
                    last = num
                elif op == '-':
                    ...
                    # Truncating division in Python 3:
                    # Use int() to truncate toward zero
                    last = int(last / num)
                
                # Reset num and record new operator
                num = 0
                op = char
...
class NestedInteger:
    def __init__(self, val=None):
        # If we’re given an int, store it directly; if a list is provided, store that list of NestedInteger
        # If val is None, default to an empty list
        if isinstance(val, int):
            self._integer = val
            self._list = None
        else:
            self._integer = None
            # If val is a list of NestedInteger, store it; otherwise an empty list
            self._list = val if isinstance(val, list) else []
...
class SparseVector:
    def __init__(self, nums: List[int]):
        """
        Build a list of (index, value) for all non-zero values in nums.
        This list is sorted by index since we iterate in ascending order.
        """
        self.pairs = []
    ...
    def dotProduct(self, vec: 'SparseVector') -> int:
        """
        Use two pointers on each pairs list. If both indices match, multiply.
        """
        result = 0
        i, j = 0, 0
        arr1, arr2 = self.pairs, vec.pairs
...
# Example usage:
# v1 = SparseVector([0,1,0,2,3])
# v2 = SparseVector([0,0,0,1,4])
# ans = v1.dotProduct(v2)
# print(ans)  # Output: 14 (because 1*0 + 2*1 + 3*4 = 2 + 12 = 14)

"""
binary search - follow up - Because each vector’s indices are sorted, a binary search correctly finds
T: O(k1logk2)
S: O(k1+k2)
"""
class SparseVector:
    def __init__(self, nums: List[int]):
        """
        Build a list of (index, value) for all non-zero values in nums.
        This is sorted by index since we iterate in order.
        """
        self.pairs = []
        for i, val in enumerate(nums):
            if val != 0:
    ...
    def dotProduct(self, vec: 'SparseVector') -> int:
        """
        For each (index, value) in self, binary search for 'index' in vec's pairs.
        If found, multiply and add to the result.
        """
        result = 0
        arr2 = vec.pairs  # the other vector's (index, value) pairs

        for (i, val1) in self.pairs:
            # Standard binary search in arr2 for index i
...
class SparseVector:
    def __init__(self, nums: List[int]):
        """
        Store the sparse vector as a list of (index, value) for non-zero entries.
        """
        self.pairs = []
        for i, val in enumerate(nums):
            if val != 0:
    ...
    def dotProductWithDense(self, dense: List[int]) -> int:
        """
        Calculate the dot product with a *dense* list.
        Complexity: O(k), where k is the number of non-zero entries in the sparse vector.
        """
        result = 0
        for idx, val in self.pairs:
            result += val * dense[idx]
        return result


# Example usage
# Suppose nums1 is sparse, nums2 is a dense list
nums1 = [0, 4, 0, 0, 2]  # sparse
nums2 = [3, 5, 7, 1, 6]  # dense

sparse_vec = SparseVector(nums1)
answer = sparse_vec.dotProductWithDense(nums2)
print(answer)  # -> 4*5 + 2*6 = 20 + 12 = 32
...
Quick Select
Time: O(n) average
- worst: O(n^2) remove element -> n + n-1 ...1 = n^2
Space: O(1)
"""
class Solution:
    ...
    def kClosest(self, points, k):
        def partition(left, right, pivot_index):
            x,y = points[pivot_index]
            pivot_value = self.distance(x,y)
            ...
            for i in range(left, right):
                ...
                left_val = self.distance(x_left, y_left)
                if left_val < pivot_value:
